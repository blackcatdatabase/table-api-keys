...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 2 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 6 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\api-keys](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\api-keys)
...
graph LR
  ...
  api_keys["api_keys"]:::center
  api_keys -->|FK| tenants["tenants"]:::link
  api_keys -->|FK| users["users"]:::accent
...
- name_ci (token)
- token_hash (token)
- token_hash_key_version (token)
- user_id (token)

## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package api-keys -Apply
