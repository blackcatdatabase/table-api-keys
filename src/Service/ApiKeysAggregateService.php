...
namespace BlackCat\Database\Packages\ApiKeys\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto;
use BlackCat\Database\Packages\ApiKeys\Mapper\ApiKeyDtoMapper;
 ...
 */
final class ApiKeysAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private ApiKeyRepository $apiKeyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'api_keys' => $this->apiKeyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\ApiKeys\ModuleException("Record {$idStr} not found for locking.");
            }
