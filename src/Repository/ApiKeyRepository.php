...
namespace BlackCat\Database\Packages\ApiKeys\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\ApiKeys\Definitions;
use BlackCat\Database\Packages\ApiKeys\Criteria;
use BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto as Dto;
use BlackCat\Database\Packages\ApiKeys\Mapper\ApiKeyDtoMapper as RowMapper;
...
final class ApiKeyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\ApiKeys\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\ApiKeys\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null */
    public function getByTokenHash(string $tokenHash, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
        $row = $this->getByUnique([ 'token_hash' => $tokenHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\ApiKeys\Mapper\ApiKeyDtoMapper::fromRow($row);
    }
    public function existsByTokenHash(string $tokenHash): bool {
        $where = 't.' . Ident::q($this->db, 'token_hash') . ' = :uniq_token_hash';
        return $this->exists($where, [ 'uniq_token_hash' => $tokenHash ]);
    }
    /** @return int|string|null */
    public function getIdByTokenHash(string $tokenHash) {
        $row = $this->getByTokenHash($tokenHash);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null */
    public function getByConstraint(mixed $constraint, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null */
    public function getByTenantIdAndNameCi(int $tenantId, string $nameCi, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'name_ci' => $nameCi ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\ApiKeys\Mapper\ApiKeyDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndNameCi(int $tenantId, string $nameCi): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'name_ci') . ' = :uniq_name_ci';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_name_ci' => $nameCi ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndNameCi(int $tenantId, string $nameCi) {
        $row = $this->getByTenantIdAndNameCi($tenantId, $nameCi);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
