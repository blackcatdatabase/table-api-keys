...
use BlackCat\Database\Packages\ApiKeys\Repository\ApiKeyRepositoryInterface;
...
class ApiKeyRepository implements ApiKeyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\ApiKeys\Definitions::class);
    ...
    public function getByTokenHash(string $tokenHash, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
        return $this->getByUnique([ 'token_hash' => $tokenHash ], $asDto);
    }
    ...
    public function getIdByTokenHash(string $tokenHash) {
        $row = $this->getByTokenHash($tokenHash, false);
    ...
    public function getByTenantIdAndNameCi(int $tenantId, string $nameCi, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'name_ci' => $nameCi ], $asDto);
    }
    ...
    public function getIdByTenantIdAndNameCi(int $tenantId, string $nameCi) {
        $row = $this->getByTenantIdAndNameCi($tenantId, $nameCi, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\ApiKeys\Dto\ApiKeyDto|null {
